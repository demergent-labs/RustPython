...
impl VirtualMachine {
    ...
    fn initialize(&mut self) {
        ...
        let mut essential_init = || -> PyResult {
            ...
            // let importlib = import::init_importlib_base(self).unwrap();
            let result = import::init_importlib_base(self);

            let importlib = match result {
                Ok(value) => Some(value),
                Err(error) => {
                    let py_object = error.clone().to_pyobject(self);
                    let err_message = match py_object.str(self) {
                        Ok(str) => str.to_string(),
                        Err(_) => error.str(self).to_string(),
                    };
                    unsafe {
                        near_sys::panic_utf8(err_message.len() as _, err_message.as_ptr() as _)
                    };
                    None
                }
            };
    ...
    fn import_inner(
        ...
        let log_message = "import_inner 0";
        ...
        let log_message = "import_inner 1";
        unsafe { near_sys::log_utf8(log_message.len() as _, log_message.as_ptr() as _) };

        let cached_module = if weird {
            None
        } else {
            let log_message = "import_inner 2";
            unsafe { near_sys::log_utf8(log_message.len() as _, log_message.as_ptr() as _) };

            let sys_modules = self.sys_module.get_attr("modules", self)?;

            let log_message = "import_inner 3";
            unsafe { near_sys::log_utf8(log_message.len() as _, log_message.as_ptr() as _) };

            sys_modules.get_item(module, self).ok()
        };

        let log_message = "import_inner 4";
        unsafe { near_sys::log_utf8(log_message.len() as _, log_message.as_ptr() as _) };

        match cached_module {
            Some(cached_module) => {
                let log_message = "import_inner 5";
                unsafe { near_sys::log_utf8(log_message.len() as _, log_message.as_ptr() as _) };

                if self.is_none(&cached_module) {
                    let log_message = "import_inner 6";
                    unsafe {
                        near_sys::log_utf8(log_message.len() as _, log_message.as_ptr() as _)
                    };
                    ...
                    let log_message = "import_inner 7";
                    unsafe {
                        near_sys::log_utf8(log_message.len() as _, log_message.as_ptr() as _)
                    };
            ...
            None => {
                let log_message = "import_inner 8";
                unsafe { near_sys::log_utf8(log_message.len() as _, log_message.as_ptr() as _) };
                ...
                let log_message = "import_inner 9";
                unsafe { near_sys::log_utf8(log_message.len() as _, log_message.as_ptr() as _) };
                ...
                let log_message = "import_inner 10";
                unsafe { near_sys::log_utf8(log_message.len() as _, log_message.as_ptr() as _) };

                let from_list = from_list.to_pyobject(self);

                let log_message = "import_inner 11";
                unsafe { near_sys::log_utf8(log_message.len() as _, log_message.as_ptr() as _) };
